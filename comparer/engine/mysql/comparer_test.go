...
func TestNewComparer(t *testing.T) {
	...
	if comparer.GetEngine() != engine.MySQL {
		t.Errorf("Expected engine MySQL, got %s", comparer.GetEngine())
	}
}

func TestMySQLComparer_Column(t *testing.T) {
	comparer := NewComparer()
	...
	tests := []struct {
		...
		{
			name: "type aliases - INT vs INTEGER",
			col1: &schemaextract.Column{
				Name:     "count",
				Type:     "INT",
				Nullable: true,
		...
		{
			name: "type aliases - BOOL vs BOOLEAN",
			col1: &schemaextract.Column{
				Name: "is_active",
				Type: "BOOL",
			},
		...
		{
			name: "type aliases - BOOL vs TINYINT(1)",
			col1: &schemaextract.Column{
				Name: "flag",
				Type: "BOOL",
			},
...
func TestMySQLComparer_Index(t *testing.T) {
	comparer := NewComparer()
...
func TestMySQLComparer_Expression(t *testing.T) {
	comparer := NewComparer()
	...
	tests := []struct {
		...
		{
			name:     "backtick identifier equivalence",
			expr1:    "`column_name` > 10",
			expr2:    "column_name > 10",
			expected: true,
		},
		{
			name:     "case insensitive keywords",
			expr1:    "status IN ('active', 'pending')",
			expr2:    "status in ('active', 'pending')",
			expected: true,
		},
		{
...
func TestMySQLComparer_Function(t *testing.T) {
	comparer := NewComparer()
	...
	tests := []struct {
	...
	}{
		{
			...
			func1: &schemaextract.Function{
				Name:       "calculate",
				Definition: "RETURNS int DETERMINISTIC BEGIN RETURN 42; END",
			},
			func2: &schemaextract.Function{
				Name:       "calculate",
				Definition: "RETURNS int DETERMINISTIC BEGIN RETURN 42; END",
		...
		},
		{
			...
			func1: &schemaextract.Function{
				Name:       "get_value",
				Definition: "RETURNS int BEGIN RETURN 1; END",
			},
			func2: &schemaextract.Function{
				Name:       "get_value",
				Definition: "RETURNS int BEGIN RETURN 2; END",
			},
			expected: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Use Equal to check if they're equal
...
func TestMySQLComparer_CanUseAlterFunction(t *testing.T) {
	comparer := NewComparer()
	funcComparer := comparer.Function()
